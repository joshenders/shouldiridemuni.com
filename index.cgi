...
# 00 00,12 * * * /usr/bin/curl -s "http://mlb.mlb.com/ticketing-client/csv/EventTicketPromotionPrice.tiksrv?team_id=137&home_team_id=137&display_in=singlegame&ticket_category=Tickets&site_section=Default&sub_category=Default&leave_empty_games=true&event_type=T&event_type=Y" | /usr/bin/cut -d, -f1,2,7,9 > schedule.csv
...
if [[ -n "${game}" ]]; then # there's a game today
    ...
    printf "<p class=\"big\">Sure.</p><p class=\"small\">The Giants aren't playing today</p>"
fi
